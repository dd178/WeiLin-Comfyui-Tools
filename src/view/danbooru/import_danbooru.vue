        ...
        <div v-if="groupSql">
          ...
          <div v-if="subGroupSql">
            ...
            <div class="tag-group-container">
              ...
              >
                <p v-if="isDanbooruFormat(tag)">
                  {{ parseDanbooruSQL(tag).tag }} - {{ parseDanbooruSQL(tag).translate }} ({{
                    t('danbooruManager.hot')
                  }}: {{ parseDanbooruSQL(tag).hot }}, {{ t('danbooruManager.aliases') }}:
                  {{ parseDanbooruSQL(tag).aliases }})
                </p>
                <p v-else>
{{ parseSQL(tag).text }} - {{ parseSQL(tag).desc }}
</p>
        ...
        <div v-else-if="tagGroups.length > 0">
          ...
          <div class="tag-group-container">
            ...
            >
              <p v-if="isDanbooruFormat(tag)">
                {{ parseDanbooruSQL(tag).tag }} - {{ parseDanbooruSQL(tag).translate }} (热度:
                {{ parseDanbooruSQL(tag).hot }}, 别名: {{ parseDanbooruSQL(tag).aliases }})
              </p>
              <p v-else>
{{ parseSQL(tag).text }} - {{ parseSQL(tag).desc }}
</p>
